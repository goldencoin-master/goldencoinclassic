...
#if defined(NDEBUG)
# error "GoldencoinClassic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb0070ca248786c1c43bd281c789f62f60add6e678325d62ece860b3680dac647");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 4); // GoldencoinClassic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GoldencoinClassic Signed Message:\n";
...
{
    // GoldencoinClassic: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // GoldencoinClassic: 210k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 60; // GoldencoinClassic: 3 blocks
static const int64 nTargetSpacing = 0.5 * 60; // GoldencoinClassic: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GoldencoinClassic: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GoldencoinClassic: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9f87c650a8ae72c6528c6e7d08f76c8861340f8e8e94df1d0f582dd91d708e32");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Goldencoin Classic";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2400 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0451f078ce11c85e1d639008b35579ae103dcdf73113cd53d5215844fec561190cf019ed022857b718d562ba72e0dc4a8f629b889732e05cc56d06ab6f4850f08d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1631807777;
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xf43fa23c17091e47f30249b162327ca8dba8e99c135591566416d048112b106c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x10, 0xb0, 0x0l, 0xbd }; // GoldencoinClassic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoldencoinClassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoldencoinClassicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoldencoinClassicMiner(CWallet *pwallet)
{
    printf("GoldencoinClassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goldencoinclassic-miner");
    ...
    try { loop {
        ...
        printf("Running GoldencoinClassicMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoldencoinClassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoldencoinClassicMiner, pwallet));
}
